...
if (isset($_POST['submit'])) {
    ...
    $sql1 = $con->prepare("SELECT * FROM empresa WHERE id_empresa = ?");
    $sql1->execute([$nit]);
    $fila = $sql1->fetch(PDO::FETCH_ASSOC);
    
    if ($fila) {
        echo '<script>alert("Ya existe el documento");</script>';
        echo '<script>window.location = "R_empresa.php";</script>';
    } else if ($nit == "" || $empresa == "") {
        echo '<script>alert("Todos los campos son obligatorios");</script>';
        echo '<script>window.location = "R_empresa.php";</script>';
    } else {
        $insert = $con->prepare("INSERT INTO empresa (id_empresa, empresa) VALUES ('$nit', '$empresa')");
        $insert->execute();    
